...
// http://iphonedevwiki.net/index.php/Logos
    ...
    #define UNIQUE_SUFFIX(stuff) CONCAT(stuff, __COUNTER__)
        ...
        #define SUBCLASS(stuff, superclass)  @implementation stuff
        #define END_SUBCLASS() @end
        #define CTOR() static void inline stuff()
        #define NEWPROPERTY(type, atype, name)
        #define NEWPROPERTY_T(type, name)
        ...
        #define END_SUBCLASS() %end
        #define CTOR() %ctor
        #define NEWPROPERTY(type, atype, name) %property (nonatomic,atype) type * name
        #define NEWPROPERTY_T(type, name) %property (nonatomic,assign) type name
